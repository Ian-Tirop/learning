...
// Shared by the standalone /admin/security page and the admin dashboard's
// Security tab — one source of truth for the whole 2FA + recovery-email UI.
export function SecurityPanel({ onStatusChange }) {
  ...
  const refresh = () => {
    ...
    Promise.all([
      ...
      .then(([twoFactorEnabled, email]) => {
        ...
        onStatusChange?.({ enabled: twoFactorEnabled, recoveryEmail: email })
      })
  ...
  return (
    <>
      ...
      <div className="security-panel-cards">
        <RecoveryEmailForm currentEmail={recoveryEmail} onSaved={setRecoveryEmailState} />
        {enabled === false && <SetupFlow onEnabled={refresh} />}
        {enabled === true && <DisableFlow onDisabled={refresh} />}
      </div>
    </>
  )
}
